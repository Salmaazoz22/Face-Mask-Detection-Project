...
- Incorrect/improper mask usage (mask pulled down or covering chin only)
...
4. Ensure the FastAPI backend is already running at `http://localhost:8000`.
...
Classifies an uploaded image and returns a mask detection result.
...
| `422`  | Image cannot be decoded or opened              |
...
01_Data_Preparation.ipynb
    ...
    │  • Organize Train / Validation / Test splits
    ▼
02_EDA.ipynb
...
04_Model_Training.ipynb
    ...
    │  • Save checkpoint → Models/mask_detector.pth
    ▼
05_evaluation_report.ipynb
    │  • Load saved checkpoint
    │  • Compute accuracy, precision, recall, F1
    │  • Confusion matrix and misclassification analysis
    │  • Export results → insights/
    ▼
api/app.py  ←──────────────────── Models/mask_detector.pth
    │  • Load model on startup (CUDA / CPU)
    │  • Expose GET /, GET /info, POST /predict
    ▼
frontend/
    │  • Webcam stream → frame capture → POST /predict
    │  • Image upload / drag-and-drop → POST /predict
    │  • Display status, action, and confidence score
    ▼
frontend/Dockerfile
    └── Serve static files via nginx:alpine on port 80
```
